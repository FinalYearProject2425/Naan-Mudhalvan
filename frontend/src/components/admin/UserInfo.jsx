...
const UserInfo = () => {
   ...
   const handleSubmit = async (user_id) => {
      if (updateUser === "") {
         ...
         axios.put(`http://localhost:5000/user/${user_id}`, updateUser)
            .then((res) => {
   ...
   useEffect(() => {
      const getOrdinaryRecords = async () => {
         try {
            const response = await axios.get('http://localhost:5000/OrdinaryUsers');
   ...
   const deleteUser = async (userId) => {
      try {
         ...
         if (confirmed) {
            await axios.delete(`http://localhost:5000/OrdinaryUsers/${userId}`);
